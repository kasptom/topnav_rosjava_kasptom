package com.github.topnav_rosjava_kasptom.topnav_graph.constants;

public class TopNavConstants {
    public static final String TOPNAV_ATTRIBUTE_KEY_ID = "topnav.id";
    public static final String TOPNAV_ATTRIBUTE_KEY_ROLE = "topnav.role";
    public static final String TOPNAV_ATTRIBUTE_KEY_COST = "topnav.cost";
    public static final String TOPNAV_ATTRIBUTE_KEY_NODE_TYPE = "topnav.nodeType";

    public static final String TOPNAV_ATTRIBUTE_VALUE_NODE_TYPE_MARKER = "markerNode";
}
