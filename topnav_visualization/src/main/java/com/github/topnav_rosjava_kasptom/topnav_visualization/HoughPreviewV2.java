...
public class HoughPreviewV2 implements IHoughPreview {
    ...
    public HoughPreviewV2(Log log) {
        ...
        this.doorFinder = new DoorFinder(new MyKMeansClustering(2, 6));
    ...
    private void updatePointsFromAngleRangeData(AngleRangesMsg angleRangesMsg) {
        ...
        if (clusters.size() == 2) {
            leftDoorPoints.addAll(AngleRangeUtils.angleRangeToPixels(clusters.get(0).stream().map(doorFinder::getAngleRangeFrom).collect(Collectors.toList()))
                    .stream().map(point2D -> new Point((int) point2D.getX(), (int) point2D.getY()))
                    .collect(Collectors.toList()));
            rightDoorPoints.addAll(AngleRangeUtils.angleRangeToPixels(clusters.get(1).stream().map(doorFinder::getAngleRangeFrom).collect(Collectors.toList()))
                    .stream().map(point2D -> new Point((int) point2D.getX(), (int) point2D.getY()))
                    .collect(Collectors.toList()));
        }
    }
